    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card>
        <div className="divide-y">
          {mine.map(n => (
            <div key={n.id} className={`p-4 flex items-start gap-4 ${!n.is_read ? "bg-accent-soft/40" : ""}`}>
              ...
              {!n.is_read && <Button size="sm" variant="ghost" onClick={() => markRead(n.id)}><Check className="h-4 w-4" /></Button>}
            </div>
          ))}
